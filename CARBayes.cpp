...
// [[Rcpp::export]]
List negbinbetaupdateRW(NumericMatrix X, const int nsites, const int p, NumericVector beta,
                         NumericVector epsilon, NumericVector offset, NumericVector y, 
                         NumericVector prior_meanbeta, NumericVector prior_varbeta, const int nblock, 
                         double beta_tune, List block_list)
{
  // Compute the acceptance probability for beta
  //Create new objects
  ...
  for(int r=0; r<nblock; r++)
  {
    ...
    for(int j = 0; j < nsites; j++)     
    {
      oldlikebit = oldlikebit + y[j] * lp_current[j] - epsilon[j] * exp(lp_current[j]);
      newlikebit = newlikebit + y[j] * lp_proposal[j] - epsilon[j] * exp(lp_proposal[j]);
    }
    likebit = newlikebit - oldlikebit;
...
// [[Rcpp::export]]
List negbincarupdateRW(NumericMatrix Wtriplet, NumericMatrix Wbegfin, 
                        NumericVector Wtripletsum, const int nsites, NumericVector phi, 
                        NumericVector epsilon, double tau2, const NumericVector y, 
                        const double phi_tune,  double rho, NumericVector offset)
{
  ...
  for(int j = 0; j < nsites; j++)
  {
    ...
    lpold = offset[j] + phinew[j];
    lpnew = offset[j] + propphi;
    oldlikebit = y[j] * lpold - epsilon[j] * exp(lpold);
    newlikebit = y[j] * lpnew - epsilon[j] * exp(lpnew);
    acceptance = exp(oldpriorbit - newpriorbit - oldlikebit + newlikebit);
...
// [[Rcpp::export]]
List poissoncarupdateRWmix(NumericMatrix Wtriplet, NumericMatrix Wbegfin, 
                           NumericVector Wtripletsum, const int nsites, NumericVector phi, 
                           double tau2, const NumericVector y, const double phi_tune, 
                           double rho, NumericVector offset, List intersect1, NumericVector n1, 
                           List intersect2, NumericVector n2, int nareas)
{
  // Update the spatially correlated random effects 
  //Create new objects
  int accept=0,rowstart=0, rowend=0;
  ...
  double priorvardenom, priormean, priorvar;
  double propphi;
  double n_obs, n_obs2, temp_old, temp_new;
  int row;
  NumericVector phinew(nsites), phicurrent(nsites);
  
  
  //  Update each random effect in turn
  phicurrent = clone(phi);
  phinew = clone(phi);
  ...
  for(int j = 0; j < nsites; j++)
  {
    ...
    rowend = Wbegfin(j,1);
    sumphi = 0;
    for(int l = rowstart; l < rowend; l++) sumphi += Wtriplet(l, 2) * phicurrent[(Wtriplet(l,1) - 1)];
    priormean = rho * sumphi / priorvardenom; 
    
    // propose a value
    proposal_var = phi_tune;
    propphi = rnorm(1, phinew[j], sqrt(proposal_var))[0];
    phinew[j] = propphi; 
    
    // Prior part of the full conditional
    newpriorbit = (0.5/priorvar) * pow((propphi - priormean), 2); 
    oldpriorbit = (0.5/priorvar) * pow((phicurrent[j] - priormean), 2);
    
    // Determine which data likelihoods to evaluate
    n_obs2 = n2[j];
    NumericVector obs2(n_obs2);
    obs2 = intersect2[j];  
    
    // Evaluate the data likelihood
    oldlikebit = 0;
    newlikebit = 0;
    // Compute the old and new poisson mean for each area k
    for(int m=0; m<n_obs2; m++)
    {
      // Determine which area you are evaluating
      int k=obs2[m] - 1;
      
      // Determine which grid squares are needed.
      n_obs = n1[k];
      NumericVector obs(n_obs);
      obs = intersect1[k];    
      
      // Compute the poisson mean for observation k
      temp_old=0;
      temp_new=0;
      for(int r=0; r<n_obs; r++)
      {
        row = obs(r, 0)-1;
        temp_old = temp_old + obs(r, 1) * exp(offset[row] + phicurrent[row]);
        temp_new = temp_new + obs(r,1) * exp(offset[row] + phinew[row]);
      }
      
      // Data likelihood part of the full conditional
      oldlikebit = oldlikebit +  y[k] * log(temp_old) - temp_old;
      newlikebit = newlikebit +  y[k] * log(temp_new) - temp_new;
    }
    ...
    if(runif(1)[0] <= acceptance) 
    {
      phicurrent[j] = phinew[j];
      accept = accept + 1;
    }
    else
    { 
      phinew[j] = phicurrent[j];
    }  
  }
